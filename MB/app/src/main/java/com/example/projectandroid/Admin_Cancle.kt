...
import kotlinx.android.synthetic.main.canclereserve.*
...
class Admin_Cancle : AppCompatActivity(), CancelOnClickListener {
    ...
    fun onClickSearch(){
        btnSearch.setOnClickListener(){
            if(SerchReserve.text.isEmpty()){
                Toast.makeText(this, "กรุณากรอกเลขที่ใบเสร็จ", Toast.LENGTH_LONG).show()
            }
            else{
                showreserve()
            }
    ...
    override fun onClickEdit(reserve: Reserve) {
        val editID  = reserve.Reserve_ID.toString()
        val editRoom = reserve.Room_Type.toString()
        val intent = Intent(this, EditDate:: class.java)
        intent.putExtra("1", idusername)
        intent.putExtra("2", username)
        intent.putExtra("3", password)
        intent.putExtra("4", address)
